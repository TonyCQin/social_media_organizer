...
after_path = base_dir / "refinement_with_hand_labels_50_v6.csv"
...
def metrics(path: Path):
    ...
    dinner_total = dinner_to_drinks = 0
    american_fp = 0
    ...
    for r in rows:
        ...
        if "american" in pset and "american" not in hset and hset:
            american_fp += 1
    ...
    return {
        ...
        "dinner_to_drinks": (dinner_to_drinks, dinner_total),
        "american_fp": american_fp,
    }
...
print("=" * 76)
print("50-LABEL METRICS: BASELINE (v4) vs UPDATED (v6)")
print("=" * 76)


def print_line(label, before_val, after_val):
    delta = after_val - before_val
    print(f"{label:<24} {before_val:>8.4f}  ->  {after_val:>8.4f}   (Δ {delta:+.4f})")
...
print(f"Dinner->Drinks: {before['dinner_to_drinks'][0]}/{before['dinner_to_drinks'][1]} -> {after['dinner_to_drinks'][0]}/{after['dinner_to_drinks'][1]}")
print(f"American FP:    {before['american_fp']} -> {after['american_fp']}")
